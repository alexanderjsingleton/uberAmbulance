...
function initialize(app, db, socket, io) {
    ...
    app.get('/ambulances', function(req, res) {
        /*
            ...
            Then, fetch the nearest ambulances using MongoDB's geospatial queries and return it back to the client.
        */
    ...
    socket.on('request-for-help', function(eventData) {
        /*
            ...
            2. AFTER saving, fetch nearby ambulances from citizen’s location
            3. Fire a request-for-help event to each of the ambulance's room
        */
        ...
        dbOperations.saveRequest(db, requestId, requestTime, location, eventData.citizenId, 'waiting', function(results) {
            ...
            //2. AFTER saving, fetch nearby ambulanes from citizen’s location
            dbOperations.fetchNearestAmbulances(db, location.coordinates, function(results) {
                eventData.requestId = requestId;
                //3. After fetching nearest ambulances, fire a 'request-for-help' event to each of them
    ...
    socket.on('request-accepted', function(eventData) { //Listen to a 'request-accepted' event from connected ambulances
        console.log(eventData);
        //Convert string to MongoDb's ObjectId data-type
        var ObjectID = require('mongodb').ObjectID;
        var requestId = new ObjectID(eventData.requestDetails.requestId);

        //Then update the request in the database with the ambulance details for given requestId
        dbOperations.updateRequest(db, requestId, eventData.ambulanceDetails.ambulanceId, 'engaged', function(results) {
            //After updating the request, emit a 'request-accepted' event to the citizen and send ambulance details
